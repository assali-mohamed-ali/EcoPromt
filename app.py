...
from energy_estimator import get_tier, savings, simple_complexity
...
with col2:
    if clarity.is_ambiguous:
        st.warning(
            "Your prompt is ambiguous. Answer a few questions to help the model understand you better."
        )
        answers = {}
...
with t2:
    ...
    if reps["repeated_words"]:
        ...
        for r in reps["repeated_words"]:
            st.write(
                f"- `{r['word']}` repeated {r['count']}× → {r['tokens_wasted']} extra token(s)"
            )
    else:
...
with t5:
    ...
    if red["redundant_pairs"]:
        st.write(
            f"Estimated **{red['estimated_removable_tokens']} removable tokens** from redundancy:"
        )
        for pair in red["redundant_pairs"]:
            st.write(
                f"- Similarity {pair['similarity']}: *\"{pair['s1']}...\"* ≈ *\"{pair['s2']}...\"*"
            )
    else:
...
st.info(
    f"Recommended open-source model: **{tier_models[tier]}** — run locally with Ollama or HuggingFace."
)
